...
const UserNftFrontSide: FC<Props> = (props: Props) => {
  ...
  //TODO: add variables
  const [readyToUpgrade, setRedyToUpgrade] = useState<boolean>(
    Math.random() >= 0.5
  );
  const rank = 69;

  // const readyToUpgrade = Math.random() >= 0.5;
      ...
      <div className="relative">
        ...
        <div className="flex gap-2 items-center text-xs md:text-sm text-cf-white/50 p-2 bg-[#0D3426] h-[38px]">
          {readyToUpgrade && !isLoadingCard ? (
            ...
            <>
              <p>cost </p>
              <p className="text-cf-white whitespace-nowrap">
                {isLoadingCard ? "" : "2500"} kira
              </p>{" "}
              {!isLoadingCard && (
                <div
                  onClick={(e) => {
